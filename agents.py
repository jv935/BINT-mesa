...
# Map knowledge from these sources is treated as verified (directly observed or
# handed out by the system) and may be shared; peer-provided knowledge is not.
...
class DeliveryAgent(CellAgent):
    """An agent that delivers packages to drop-off locations.

    When it does not know where its current goal drop-off is, it asks other
    agents for the coordinate, deciding whom to trust from their TNFT history
    (and, when staking is enabled, negotiating collateral). It can in turn share
    its own verified drop-off knowledge with others.
    """
    ...
    def __init__(
        ...
        :param vision_radius: How far the agent perceives each step.
        :param trust_reject_threshold: Score at or below which trust is always rejected.
        :param trust_accept_threshold: Score at or above which trust is always accepted.
        """
        ...
        # Peer-provided coordinates -- usable by this agent but not shared until
        ...
        # Diagnostics describing this agent's most recent trust decision.
    ...
    def record_decision(
        ...
        """Record the latest trust decision on this agent and tally its reason on
        the model. The full per-event list is only built when the model has
        DEBUG_DECISIONS set (kept off during production runs)."""
    ...
    def verify_credibility(self, reviewer_id: str) -> bool:
        """Whether a reviewer's negative-review rate is low enough to trust their
        reports. Reviewers with too few reviews pass by default."""
    ...
    def calculate_trust_summary(
        ...
        score_cache = getattr(self.model, "_trust_score_cache", None)
        if score_cache is not None:
            cache_key = (self.unique_id, target_id, service_type, use_filter)
        ...
        result = self.calculate_trust_summary_from_evidence(
            evidence=evidence,
            filter_untrusted_evidence=use_filter,
        )
    ...
    def calculate_trust_summary_from_evidence(
        ...
        """Turn raw ledger evidence into a trust score.

        Active tokens raise the score, burned tokens lower it, each weighted by
        whether it matches the service context. The score is a smoothed ratio of
        weighted-active to total weighted evidence using the agent's priors.
        """
        if filter_untrusted_evidence is None:
    ...
    def _filter_untrusted_trust_evidence(
        ...
        """Keep only the tokens whose issuer (for active) or burner (for burned)
        this agent itself trusts, then recount."""
    ...
    def _trusts_evidence_source(
        ...
        """Whether this agent accepts evidence created by ``source_id``.

        Uses a hard reject-threshold rule (not the probabilistic acceptance rule)
        so that evidence filtering is deterministic. SYSTEM and the agent itself
        are always trusted.
        """
    ...
    def _build_trust_evidence_from_tnfts(
        ...
        """Re-derive the counts in an evidence dict from explicit token lists
    ...
    def _accepts_trust_score(self, score: float) -> bool:
        """Decide whether to accept a peer with this trust score.

        Below the reject threshold: never. At/above the accept threshold: always.
        In between: accept with probability that rises linearly across the band.
        """
    ...
    def _trust_score_to_risk(self, trust_score: float) -> float:
        """Convert a trust score into a bounded risk (high trust -> low risk).
    ...
    def _reviewer_summary_to_risk(self, reviewer_summary: dict[str, Any]) -> float:
        """Convert reviewer history into a bounded risk. Reviewers with too few
        reviews contribute no risk."""
    ...
    def calculate_interaction_risk(
        ...
        """How risky another agent looks in a given role, combining trust-score
        risk and reviewer-history risk by this agent's role weights."""
    ...
    def _stake_fraction_from_risk(self, risk: float) -> float:
        """Risk -> the fraction of stake this agent demands (more risk, more stake)."""
    ...
    def _stake_amount_from_risk(self, risk: float, available_stake: int) -> int:
        """Risk -> a concrete required stake count, relative to what's available."""
        if not getattr(self.model, "staking_enabled", False):
            return 0
    ...
    def calculate_required_stake(
        ...
        """How much stake this agent requires from another agent."""
    ...
    def _stake_limit_fraction_from_risk(self, risk: float) -> float:
        """Peer risk -> the maximum fraction of its own stake this agent will
        risk (more peer risk, less willing)."""
        if not getattr(self.model, "staking_enabled", False):
            return 0.0
    ...
    def calculate_stake_limit(
        ...
        """The most stake this agent is willing to risk with a peer: more for a
        trustworthy peer, less for a risky one."""
        risk_summary = self.calculate_interaction_risk(
            agent_id=peer_id,
            role=peer_role,
            service_type=service_type,
    ...
    def accepts_stake_offer(
        ...
        """Whether another agent's stake offer meets this agent's requirement."""
        if not getattr(self.model, "staking_enabled", False):
            return True
    ...
    def build_map_share_response(
        ...
        """Package a coordinate together with the staking terms this agent offers
        the requester."""
    ...
    def accepts_requester_for_service(
        ...
        """Whether to serve a requester: gate on their trust score, then on their
        reviewer credibility. Logs the reason either way."""
    ...
    def share_map(self, requester: CellAgent, target: str) -> MapShareResponse | None:
        """Honestly share a verified drop-off coordinate with an accepted
        requester, or decline (None)."""
    ...
    def evaluate_map_response_stakes(
        ...
        """Decide whether the staking terms in a provider's response are
        acceptable to both sides, and how much each should actually lock."""
        ...
        return {
            ...
            # Lock exactly the requirement, not the full offered ceiling;
            # provider_willing_to_stake already guarantees the offer covers it.
    ...
    # ----- Receiving and acting on map data ------------------------------- #
    def _try_get_goal_location_from_other_agent(self) -> bool:
        """Ask other agents for the current goal's coordinate; accept the first
        acceptable offer (responses arrive nearest-first)."""
    ...
    def _accept_map_response(self, response: dict[str, Any]) -> bool:
        """Decide whether to accept one provider's offer: gate on trust score,
        reviewer credibility, and staking terms, then open a staked interaction
        and adopt the provided coordinate."""
        if self.goal_name is None:
            return False
        ...
        if self._get_goal_coordinate(self.goal_name) is None:
            # This agent already has verified knowledge that the coordinate is
            # not this drop-off, so the provider's info is provably wrong; treat
            # it exactly like arriving at the wrong destination.
    ...
    def move(self) -> bool:
        """Take one step toward the current target. Returns whether the agent
        actually moved."""
    ...
    def _handle_bad_delivery_target(self) -> None:
        """A provider's coordinate turned out not to be the goal drop-off: settle
        the interaction as a failure and discard the bad coordinate."""
        ...
        # Keep self/system-verified knowledge about this coordinate; only drop a
        # stale provider-created record.
    ...
    def decide_reported_outcome(
        ...
        """Honest agents report the true outcome. (Overridden to lie by the
        malicious subclass.)"""
    ...
    def _settle_current_interaction(
        ...
        """Report the (possibly falsified) outcome of the current interaction to
        the model and clear it."""
        interaction_id = self.current_interaction_id

        if interaction_id is None:
            return

        # Clear the reference before notifying the model so this method is
        # self-contained and cannot double-settle.
    ...
    def update_internal_map(
        ...
        """Record what is at a coordinate.

        Self/system knowledge is verified; peer knowledge is treated as a
        candidate until verified. Verified records are never overwritten by
        unverified ones, but direct observation may correct an earlier candidate.
        """
        ...
        # Directly observing floor means no drop-off should point here.
        ...
        # drop-off, not to any other name that previously claimed it.
        if (
            info_source == SOURCE_SELF
    ...
    def perceive_env(self) -> None:
        """Look within vision range and record drop-offs and floor seen."""
    ...
    def select_unexplored_coordinate(self) -> Coordinate | None:
        """Pick a random not-yet-seen coordinate, or None if all are known."""
    ...
    def _current_delivery_target_invalidated(self) -> bool:
        """True if perception has disproved the provider map this delivery is
        based on: an accepted interaction whose goal coordinate no longer matches
        the target the agent is walking toward."""
        if self.state != "DELIVERING":
            return False
...
class DropOffLocationAgent(FixedAgent):
    """A fixed agent marking a drop-off location."""
...
# --------------------------------------------------------------------------- #
# Malicious delivery agent
# --------------------------------------------------------------------------- #
class MaliciousDeliveryAgent(DeliveryAgent):
    """A delivery agent that, with configurable probabilities, fabricates map
    coordinates and forges the reviews it leaves."""
    ...
    @override
    def share_map(self, requester: CellAgent, target: str) -> MapShareResponse | None:
        """Either fabricate a coordinate (with probability false_map_probability)
        or behave exactly like an honest agent.

        false_map_probability governs ALL fabrication, for known and unknown
        targets alike, so that 0.0 is a clean "no map attack" baseline (a
        non-attacking malicious agent declines unknown targets honestly) and 1.0
        always fabricates.
        """
        ...
        if not self._draw_probability(self.false_map_probability):
            # Not lying this interaction: behave honestly.
    ...
    def decide_reported_outcome(
        ...
        """Possibly flip the reported outcome: a real success reported as failure
        (false negative) or a real failure reported as success (false positive),
