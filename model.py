...
# Delivery scoring. A delivery earns full points if it arrives within the grace
# window, decays linearly to zero up to the deadline, and is penalised per step
# once late (capped at twice the base reward).
...
class InteractionRecord:
    """A trust interaction between a truster and a trustee, e.g. a request for
    map data. Created pending and resolved by an outcome."""
...
@dataclass
class OutcomeRecord:
    """The reported result of an interaction."""
...
@dataclass
class AgentProfile:
    """A request to spawn ``count`` agents of ``agent_class`` with ``kwargs``."""
    ...
    def __post_init__(self) -> None:
        ...
        if not issubclass(self.agent_class, DeliveryAgent):
            raise TypeError("AgentProfile: agent_class must be a DeliveryAgent subclass.")
...
class BintWorldModel(mesa.Model):
    ...
    Delivery agents move across a grid taking packages to named drop-off
    locations. An agent that does not know where its goal is may ask others for
    the coordinate; the model mediates these exchanges and records them as trust
    interactions. Trust is expressed through non-transferable tokens (TNFTs)
    minted on success and burned on failure, optionally backed by staked
    collateral.

    The model owns the TNFT ledger and the interaction/outcome records. Several
    derived indexes and memoisation caches are maintained alongside this primary
    state purely for speed; they hold no information that cannot be recomputed
    from the ledger and records.
    """
    ...
    def _initialise_ledger_and_records(self) -> None:
        """Set up primary state (ledger, interactions, outcomes) and the derived
        indexes/caches that accelerate reads against it.

        Primary state:
            tnft_ledger          every minted token, mutated in place on burn/stake
            interactions         interaction_id -> InteractionRecord
            outcomes             interaction_id -> OutcomeRecord

        Derived (recomputable) state, kept only for performance:
            ledger_by_owner          tokens grouped by current owner
            staked_by_interaction    staked tokens grouped by interaction
            outcome counters         running success/failure/false-review totals
            reviewer_cache           memoised get_reviewer_summary results
            agent_index              unique_id -> delivery agent
            dropoff_index            unique_id -> drop-off agent
            decision_counts          running tally of decision reasons
            trust_evidence_cache     memoised raw ledger evidence per agent
            trust_score_cache        memoised per-evaluator trust scores
            tokens_by_issuer /       reverse indexes used to invalidate the
            tokens_burned_by         trust_score_cache precisely
        """
        # Primary state.
        ...
        # Populated in _lock_stake; consumed and removed in
        # release_interaction_stakes / burn_interaction_stakes.
        self._staked_by_interaction: defaultdict[str, list[dict[str, Any]]] = defaultdict(list)

        # Running outcome totals, read directly as metrics.
        self._n_success: int = 0
        self._n_failure: int = 0
        self._n_false_reviews: int = 0

        # Memoised reviewer summaries; invalidated per reviewer in record_outcome.
        self._reviewer_cache: dict[str, dict] = {}

        # Built in _cache_agents once all agents exist.
        self._agent_index: dict[str, 'DeliveryAgent'] = {}
        self._dropoff_index: dict[str, 'DropOffLocationAgent'] = {}

        # Running tally of decision reasons (the full event list is only built
        # when DEBUG_DECISIONS is set on the agent side).
        self._decision_counts: dict[str, int] = {}

        # Memoised raw trust evidence, keyed by (agent_id, service_type).
        self._trust_evidence_cache: dict[tuple, dict] = {}

        # Memoised trust scores, keyed by
        # (evaluator_id, target_id, service_type, filter_flag). The reverse
        # indexes below let _invalidate_trust_score_cache drop only the entries a
        # token-state change could have affected (see that method for the rules).
    ...
    def _cache_agents(self) -> None:
        """Snapshot the spawned agents into typed lists and lookup indexes."""
    ...
    def record_staked_interaction(
        ...
        """Create an interaction and lock both sides' stake.

        Returns the interaction id on success, or None if either side cannot
        provide its requested stake (in which case any partial lock is undone).
        """
    ...
    def record_outcome(
        ...
        # The reviewer is the truster who reported this outcome; drop their
        # memoised summary so it is recomputed on next read.
    ...
    def settle_interaction(
        ...
        """Record an outcome for a pending interaction and apply its effects."""
    ...
    def _apply_outcome_to_interaction(
        ...
        if outcome.status == "failure":
            if self.staking_enabled:
                # The outcome is recorded regardless of how many staked tokens
                # were actually burned, so this always completes; "cancelled" is
                # reserved for settlements that could not proceed at all.
                self.burn_interaction_stakes(
        ...
        # Any other status (e.g. disputed): release stake and cancel.
        self.release_interaction_stakes(interaction.interaction_id)
    ...
    def mint_tnft(
        ...
        tnft = {
            ...
            "interaction_id": interaction_id,
            "metadata": meta or {},
            "status": True,  # True = active, False = burned
            "timestamp": self.time,
            "staked_for": None,
            "stake_role": None,
        ...
        self._ledger_by_owner[receiver_id].append(tnft)

        # A new active token changes the receiver's score and is recorded as
        # issued-by issuer_id; refresh the affected caches.
        self._tokens_by_issuer[issuer_id].add(receiver_id)
        self._invalidate_trust_score_cache(receiver_id)
        self._invalidate_trust_evidence(receiver_id)

        receiver_agent = self._find_delivery_agent(receiver_id)
        if receiver_agent is not None:
            receiver_agent.cached_active_tnfts += 1
    ...
    def burn_tnft(
        ...
        """Burn one of ``target_id``'s active tokens (non-staked failure path).

        Prefers a token matching ``service_type``, falling back to a bootstrap
        token, and burns the bootstrap-first / lowest-id candidate. Returns
        whether a token was burned.
        """
        active_tnfts = [
            tnft
        ...
        self._invalidate_trust_evidence(target_id)
    ...
    def _invalidate_trust_score_cache(self, changed_agent_id: str) -> None:
        """Drop trust-score cache entries a token-state change could affect.

        Cache keys are (evaluator_id, target_id, service_type, filter_flag).
        When ``changed_agent_id``'s tokens change, three groups become stale:
          (a) any score whose target IS changed_agent_id (their counts changed);
          (b) filtered scores whose target holds a token ISSUED BY changed_agent_id
              (its issuer's trust is re-checked during evidence filtering);
          (c) filtered scores whose target had a token BURNED BY changed_agent_id
              (burn evidence is filtered by the burner's identity).
        Unfiltered scores for other targets depend only on the target's own
        counts and are therefore left untouched.
        """
        ...
        stale_keys = [
            key
            for key in self._trust_score_cache
            if key[1] == changed_agent_id
            or (key[3] and key[1] in indirect_targets)
        ]
        for key in stale_keys:
            del self._trust_score_cache[key]

    def _invalidate_trust_evidence(self, agent_id: str) -> None:
        """Drop cached raw evidence for an agent across all service-type keys."""
        stale_keys = [key for key in self._trust_evidence_cache if key[0] == agent_id]
        for key in stale_keys:
            del self._trust_evidence_cache[key]
    ...
    def get_available_stake_tnfts(
        ...
        """Active, unstaked tokens usable as collateral.

        Staked tokens still count toward reputation but cannot back another
        pending interaction. When a service type is given, tokens are ordered
        matching-first, then bootstrap, then everything else.
        """
        available_tnfts = [
            tnft
    ...
    def _lock_stake(
        ...
        """Reserve ``amount`` tokens as collateral. Returns the locked token ids,
        or None if the agent does not have enough available."""
        amount = int(amount)
    ...
    def release_interaction_stakes(self, interaction_id: str) -> None:
        """Unlock all still-active tokens staked for an interaction."""
        for tnft in self._staked_by_interaction.pop(interaction_id, []):
            if not tnft["status"]:  # burned between stake and release
                continue
    ...
    def burn_interaction_stakes(
        ...
        """Burn all still-active tokens staked for an interaction. Returns the
        number burned."""
        burned_count = 0

        for tnft in self._staked_by_interaction.pop(interaction_id, []):
            if not tnft["status"]:  # already burned by another path
                continue
            ...
            owner = tnft["owner"]
            self._tokens_burned_by[burner_id].add(owner)
            self._invalidate_trust_score_cache(owner)
            self._invalidate_trust_evidence(owner)

            target_agent = self._find_delivery_agent(owner)
            if target_agent is not None:
                target_agent.cached_active_tnfts = max(0, target_agent.cached_active_tnfts - 1)
                target_agent.cached_burned_tnfts += 1
    ...
    def get_vtp(
        ...
        """Return an agent's tokens, optionally filtered to active and/or a
        single service type. ``sort`` (newest first) may be skipped in hot paths
    ...
    def get_trust_evidence(
        ...
        """Return the raw ledger evidence for an agent, partitioned into active
        vs burned and context-matching vs other-service tokens.

        The model owns the ledger and so is responsible for gathering and
        grouping evidence; agents are responsible for turning it into a score.
        """
        cache_key = (agent_id, service_type)
        if cache_key in self._trust_evidence_cache:
            return self._trust_evidence_cache[cache_key]
        ...
        self._trust_evidence_cache[cache_key] = evidence
        return evidence
    ...
    def get_vtp_summary(
        ...
        """Return a trust summary for an agent from ``evaluator``'s perspective.

        The score uses the evaluator's own trust parameters, so an evaluator is
        required: there is no neutral fallback that would match a specific
        agent's live decisions.
        """
    ...
    def query_vtp(self, agent_id: str) -> int:
        """Legacy active-token count. Prefer trust evidence or agent-calculated
        summaries in new code."""
        return self.get_trust_evidence(agent_id)["total_active"]

    def get_reviewer_summary(self, reviewer_id: str) -> dict[str, Any]:
        """Summarise the outcomes a reviewer (truster) has reported.

        Memoised; the cache entry is dropped in record_outcome whenever this
        reviewer reports a new outcome.
        """
    ...
    def request_map_data(
        ...
        """Ask every other agent for ``target_name``'s coordinate and return the
        offers, nearest provider first."""
        responses = []
            ...
            responses.append(
                {
                    ...
                    "provider_stake_limit_meta": agent_resp["provider_stake_limit_meta"],
                    "requester_stake_required_meta": agent_resp["requester_stake_required_meta"],
                }
            )
    ...
    def distribute_initial_knowledge(self) -> None:
        """Hand out drop-off coordinates evenly as starting knowledge.

        Every delivery agent learns at least one drop-off before any agent gets a
        second; if there are fewer drop-offs than agents, some start with none.
        """
    ...
    def dispatch_packages(self) -> None:
        """Assign a fresh delivery goal to every idle agent, never repeating an
        agent's immediately previous goal."""
        if not self.cached_delivery_agents or not self.cached_drop_offs:
            return

        idle_agents = self._agents_without_packages()
        if not idle_agents:
            return
        ...
        for agent in idle_agents:
            ...
            max_steps_to_destination = int(min_steps_to_destination * 2)
    ...
    def verify_delivery(self, agent: DeliveryAgent, package: dict[str, Any]) -> bool:
        """Score a delivery if the agent is on its goal cell.

        Full points within the grace window; linear decay to zero up to the
        deadline; a per-step penalty once late (capped at -2x base). Returns
        whether the agent was actually at its goal.
        """
